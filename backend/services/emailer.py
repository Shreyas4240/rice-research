...
student to a professor. Uses Gemini when GEMINI_API_KEY is set, otherwise
...
- You must ONLY output the final email text.
- Do NOT output any reasoning, chain-of-thought, or <think> tags.
- Do NOT wrap your output in markdown code blocks like ```email or ```text.
- Do NOT output any conversational filler like "Here is the email draft:".
...
def _parse_draft(raw: str) -> dict[str, str]:
    """Split the raw LLM output into subject and body, stripping reasoning."""
    # Strip <think> blocks even if the model leaks them despite instructions
...
I am {student_name}, a {year_str}{major} student at Rice. I came across your research on {research_snippet}… and it resonates strongly with my interest in {interests_snippet}.
...
I hope this message finds you well! My name is {student_name}, and I'm a {year_str}{major} student at Rice. I recently read about your work on {research_snippet}… and found myself genuinely excited — it connects closely with what I've been exploring in {interests_snippet}.
...
My name is {student_name}, and I am a {year_str}{major} student at Rice. I have been exploring research opportunities in {interests_snippet} and came across your work on {research_snippet}…, which I found particularly compelling.
...
def generate_draft(
    ...
    try:
        ...
        print(f"[emailer] Gemini LLM call failed, using template")
        return _template_draft(prof, resume_profile, interests, tone)
