...
def _mock_explanation(prof: dict, interests: str, resume_profile: dict) -> str:
    ...
    # Check if professor has Google Scholar data
    has_scholar = prof.get("scholar_interests") and len(prof.get("scholar_interests", [])) > 0
    scholar_note = "" if has_scholar else " Note: Research interests based on available summary data."

    return (
        f"Prof. {name}'s research in {dept} focuses on {research}…, "
        f"which aligns with your interest in {interest_snippet}.{resume_hint}{scholar_note}"
    )
...
def match_faculty(
    ...
    """
    Rank faculty by fit and return top_n results with scores and fit labels.
