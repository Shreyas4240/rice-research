...
export default function EmailModal({ prof, session, onClose }) {
  ...
  return (
    ...
    >
      <div className="w-full max-w-2xl bg-ricewhite-50 rounded-2xl border border-ricewhite-300
        ...
        <div className="flex items-start justify-between gap-3 px-6 pt-6 pb-4
          ...
          <div className="min-w-0">
            <div className="text-[11px] font-semibold text-riceblue-700 uppercase
          ...
          <button onClick={onClose}
                  className="flex-shrink-0 p-2 rounded-xl text-stone-400 hover:text-stone-700
                             hover:bg-ricewhite-200 transition-colors">
        ...
        {/* Tone selector */}
        <div className="px-6 py-3 border-b border-ricewhite-300 flex items-center gap-2 flex-wrap">
          ...
          {TONES.map(t => (
            <button key={t.id} onClick={() => handleToneChange(t.id)} disabled={loading}
                    className={`px-4 py-1.5 rounded-lg text-xs font-semibold transition-all border ${
                      tone === t.id
                        ? 'bg-riceblue-700 text-ricewhite-100 border-riceblue-700'
                        : 'border-ricewhite-400 text-stone-600 hover:border-riceblue-300 hover:text-riceblue-700'
                    } disabled:opacity-40`}>
              {t.label}
            </button>
          ))}
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
          {loading && (
            <div className="flex flex-col items-center justify-center py-16 text-center">
              <div className="w-7 h-7 border-2 border-riceblue-700 border-t-transparent
          ...
          {draft && !loading && (
            <>
              <div>
                ...
                <div className="text-sm font-medium text-stone-800 bg-ricewhite-100
                                border border-ricewhite-300 rounded-xl px-4 py-2.5">
                  {draft.subject}
                </div>
              </div>
              <div>
                ...
                <textarea
                  ...
                  rows={14}
                  className="w-full text-sm text-stone-800 bg-white border border-ricewhite-300
                             rounded-xl px-4 py-3 leading-relaxed resize-y font-sans
                             focus:outline-none focus:ring-2 focus:ring-riceblue-700/25
                             focus:border-riceblue-700 transition-colors"
                />
              </div>
            </>
          )}
        ...
        {/* Footer */}
        <div className="px-6 py-4 border-t border-ricewhite-300 flex items-center
                        justify-between gap-3 bg-ricewhite-100/80">
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            <button onClick={copy} disabled={!draft || loading}
                    className="flex items-center gap-2 px-5 py-2 bg-riceblue-700 text-ricewhite-100
                               text-sm font-semibold rounded-xl hover:bg-riceblue-600
