...
export function AppProvider({ children }) {
  ...
  const [saved, setSaved] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem('rice_saved_profs') || '[]')
    } catch {
      return []
    }
  })
  ...
  function toggleSave(id) {
    setSaved(prev => {
      const next = prev.includes(id)
        ? prev.filter(x => x !== id)
        : [...prev, id]
      localStorage.setItem('rice_saved_profs', JSON.stringify(next))
      return next
    })
  }
  ...
  function clearSaved() {
    setSaved([])
    localStorage.removeItem('rice_saved_profs')
  }
